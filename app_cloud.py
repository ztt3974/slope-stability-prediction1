# -*- coding: utf-8 -*-
"""
边坡稳定性预测系统 - Streamlit Web应用 (云端部署版)
基于IPSO-BP优化集成学习模型
...
training_script_dir = os.path.join(base_dir, 'training_scripts')

sys.path.insert(0, training_script_dir)

try:
    from ipso_bp_slope_stability import create_features
except ImportError:
    st.error("""
        ❌ **训练脚本未找到**
        
        请确保 `training_scripts/ipso_bp_slope_stability.py` 文件存在。
        
        如果您只需要预测功能（不需要重新训练），可以使用简化版应用。
    """)
    st.stop()
...
MODEL_FILENAME = 'ipso_bp_ensemble_model.pkl'
...
def load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        return None, False, f"""
            ...
            **解决方案**:
            1. 将训练好的模型文件 `{MODEL_FILENAME}` 放入 `model/` 目录
            2. 或运行训练脚本: `python training_scripts/train_model.py`
    ...
    try:
        ...
        return None, False, f"""
            ...
            **原因**: 模型文件包含 Windows 特殊字符，无法在当前环境加载
...
def main():
    ...
    **基于IPSO-BP优化的集成学习模型**  
    ...
    if not model_loaded:
        st.error(model_msg, icon="🚫")
        st.info("💡 **提示**: 首次部署时需要先准备模型文件。请查看下方的部署说明。")
        
        with st.expander("📋 首次部署指南", expanded=True):
            st.markdown("""
            ### 需要准备的文件
            
            1. **模型文件** (`model/ipso_bp_ensemble_model.pkl`)
               - 从本地训练获得，或联系开发者获取
            
            2. **训练脚本** (`training_scripts/ipso_bp_slope_stability.py`)
               - 包含特征工程函数和模型定义
            ...
            ├── model/
            │   └── ipso_bp_ensemble_model.pkl  # 训练好的模型
            └── training_scripts/
                └── ipso_bp_slope_stability.py  # 训练脚本
            ```
